...
async def get_agenda(date_str: str, user: dict = None) -> list[dict]:
    ...
    # Looking at structured column data_retorno_agendado or fallback to text matching
    conditions = [
    ...
    params = [date_str, f"%{target_date_formatted}%", f"%{date_str}%", f"%{target_date_formatted}%"]

    if user and user.get("role") == "consultor":
        # Strictly filter by consultant's email (no unassigned leads)
    ...
    for row in rows:
        ...
        # Check structured columns first
        time_str = row.get("horario_retorno_agendado") or ""
        event_type = row.get("tipo_retorno") or ""
        
        # Fallback to regex for legacy calls
        if not event_type or not time_str:
            fallback_event_type = "Retorno"
            if reuniao_field and str(reuniao_field).lower() != 'none' and str(reuniao_field).strip() != '':
                fallback_event_type = "Reunião"
            ...
            if match:
                matched_type = match.group(1).strip().capitalize()
                if not event_type:
                    event_type = matched_type
                if not time_str and match.group(2):
...
async def add_agenda_comment(phone: str, date_str: str, comment: str, user_email: str) -> dict:
    ...
    if tag_name in ("Tarefa", "Chamada") and date_val:
            ...
            await query(
                "INSERT INTO chamadas (nome_contato, telefone_normalizado, data_hora, resumo_ligacao, status_ligacao, anotacoes, source_file, data_retorno_agendado, horario_retorno_agendado, tipo_retorno) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (lead_name, phone, created_at, resumo, f"{tag_name} Agendada", f"Agendamento automático via tag por {user_email}", "CRM_MANUAL", date_val, time_str, tag_name)
            )
...
async def reschedule_agenda_item(phone: str, lead_name: str, new_date_str: str, new_time_str: str, user_email: str, comment: str = None) -> bool:
    try:
    ...
    if comment:
        resumo += f" - Observação: {comment}"
    
    await query(
        "INSERT INTO chamadas (nome_contato, telefone_normalizado, data_hora, resumo_ligacao, status_ligacao, anotacoes, source_file, data_retorno_agendado, horario_retorno_agendado, tipo_retorno) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (lead_name, phone, created_at, resumo, "Reagendamento CRM", user_email, "CRM_MANUAL", new_date_str, new_time_str, "Retorno")
    )
    return True
